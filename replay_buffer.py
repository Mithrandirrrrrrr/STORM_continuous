...
class ReplayBuffer():
    def __init__(self, action_dim, obs_shape, num_envs, max_length=int(1E6), warmup_length=50000, store_on_gpu=False) -> None:
    ...
    def append(self, obs, action, reward, termination):
        ...
        else: # self.last_pointer == self.max_length - 1
            assert self.last_pointer == self.max_length / self.num_envs - 1, "buffer full but last_pointer is not at the end"
            self.obs_buffer[0:-1] = self.obs_buffer.clone()[1:]
            self.action_buffer[0:-1] = self.action_buffer.clone()[1:]
            self.reward_buffer[0:-1] = self.reward_buffer.clone()[1:]
            self.termination_buffer[0:-1] = self.termination_buffer.clone()[1:]
