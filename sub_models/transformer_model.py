...
class StochasticTransformer(nn.Module):
    ...
    def forward(self, samples, action, mask):
        ...
        # print("StochasticTransformer.forward_with_kv_cache", samples.shape, action.shape)
...
class StochasticTransformerKVCache(nn.Module):
    ...
    def forward(self, samples, action, mask):
        ...
        '''
        # action = F.one_hot(action.long(), self.action_dim).float()
        # print("StochasticTransformerKVCache.forward", samples.shape, action.shape)
        feats = self.stem(torch.cat([samples, action], dim=-1))
        feats = self.position_encoding(feats)
        feats = self.layer_norm(feats)
    ...
    def forward_with_kv_cache(self, samples, action):
        ...
        # action = F.one_hot(action.long(), self.action_dim).float()
        # print("StochasticTransformerKVCache.forward_with_kv_cache", samples.shape, action.shape) # torch.Size([1024, 1, 1024]) torch.Size([1024, 1, 6])
        feats = self.stem(torch.cat([samples, action], dim=-1))
