...
from sub_models.functions_losses import SymLogTwoHotLoss, symexp
...
class WorldModel(nn.Module):
    ...
    def calc_last_dist_feat(self, latent, action):
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=self.use_amp):
            # print("calc_last_dist_feat", action.shape) # torch.Size([1, 6])
    ...
    def predict_next(self, last_flattened_sample, action, log_video=True):
        with torch.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=self.use_amp):
            # print("predict_next", action.shape) # torch.Size([1024, 1, 6])
    ...
    # for training agent
    @torch.no_grad()
    def imagine_data(self, agent: SoftActorCriticAgent, sample_obs, sample_action,
        ...
        for i in range(imagine_batch_length):
            action, log_prob, z, mean, log_std = agent.sample(torch.cat([self.latent_buffer[:, i:i+1], self.hidden_buffer[:, i:i+1]], dim=-1))
    ...
    # for planning
    @torch.no_grad()
    def estimate_value(self, agent:SoftActorCriticAgent, sample_obs, sample_action, agent_actions, gamma, 
                     imagine_batch_size, imagine_batch_length):
        self.init_imagine_buffer(imagine_batch_size, imagine_batch_length, dtype=self.tensor_dtype)
        traj_value = torch.zeros(agent_actions.shape[0], dtype=self.tensor_dtype, device="cuda")
        ...
        # imagine
        for i in range(imagine_batch_length):
            self.action_buffer = agent_actions

            last_obs_hat, last_reward_hat, last_termination_hat, last_latent, last_dist_feat = self.predict_next(
                self.latent_buffer[:, i:i+1], self.action_buffer[:, i:i+1], log_video=False)

            self.latent_buffer[:, i+1:i+2] = last_latent
            self.hidden_buffer[:, i+1:i+2] = last_dist_feat
            traj_value += gamma** i * last_reward_hat.squeeze() * (1 - last_termination_hat.squeeze().int())
            # traj_value += gamma** i * last_reward_hat.squeeze()

        latent = torch.cat([last_latent, last_dist_feat], dim=-1)
        last_action, last_log_prob, last_z, last_mean, last_log_std = agent.sample(latent)
        last_latent_value1 = agent.critic1(latent, last_action).squeeze() # torch.Size([num_samples])
        last_latent_value2 = agent.critic2(latent, last_action).squeeze()
        last_latent_value = torch.min(last_latent_value1, last_latent_value2) # torch.Size([num_samples])
        return traj_value + gamma ** imagine_batch_length * last_latent_value * 0.6
